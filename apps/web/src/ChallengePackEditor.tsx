...
export function ChallengePackEditor({
  ...
  async function run(
    ...
    try {
      ...
      onError(
        caught instanceof ApiError ? caught.message : 'Could not save the pack.',
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <Panel
      ...
      aside={isOfficial ? 'official' : `private v${String(event.challengePackVersion ?? 1)}`}
    >
      <p className="text-muted mb-3 text-sm">
        Only predefined primitives. Saving writes a new version and never
        mutates a pack already used by this event.
      </p>
      ...
      {isOfficial ? (
        <p className="text-muted text-sm">
          Copy the official pack or start blank before editing.
        </p>
      ) : (
        <>
          <Field
          ...
          <Button
            ...
            onClick={() =>
              setDraft({
                ...
                challenges: [
                  ...draft.challenges,
                  blankChallenge(`claim-${String(draft.challenges.length + 1)}`),
                ],
              })
            }
          >
            ...
            onChange({
              ...
              confirmationQuestion:
                primitive.type === 'players_eliminated'
                  ? challenge.confirmationQuestion ?? 'Did this happen at the table?'
                  : undefined,
            });
          }}
        >
...
function blankChallenge(id: string): Challenge {
  return {
    id,
    name: 'New claim',
